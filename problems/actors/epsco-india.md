---
name: EPSCO India (Environmental Protection Service Company (India) Pvt. Ltd.)
slug: epsco-india
type: org
depth: tracked
aka: ["EPSCO", "Environmental Protection Service Company India"]
leg: [enterprise]
affected_led: no
representation_unit: enterprise
stance: works-the-remedy
leaves: [ambient-asbestos-demolition-dust]
nodes: [toxic-exposure-class]
geography: [india]
lifecycle: operating
lifecycle_as_of: 2026-09-07
needs:
  - {kind: policy-access, text: "a pre-demolition asbestos-survey / licensed-abatement mandate in the C&D Waste Rules — without it there is no procurement trigger outside voluntary corporate clients", as_of: 2026-09-07, source: "inferred", state: open}
offers:
  - {kind: fieldwork, text: "licensed asbestos abatement, inspection, testing and safe disposal — the removal capacity the demolition-dust failure has no institutional trigger for"}
  - {kind: training, text: "US-certified asbestos inspector/planner/contractor training (NAETI); an in-country trainer for a workforce that has none"}
sources:
  - {kind: website, url: "https://epscoindia.com/", handle: "epscoindia.com — blog updates on asbestos abatement", last_checked: 2026-09-07, status: live}
  - {kind: website, url: "https://epscoindia.com/about-us/", handle: "about / services", last_checked: 2026-09-07, status: live}
  - {kind: other, url: "https://nadca.com/find-a-professional/epsco-india-pvt-ltd-0", handle: "NADCA directory — member since 2017-05-03", last_checked: 2026-09-07, status: live}
  - {kind: linkedin, url: "https://www.linkedin.com/company/epsco-india", handle: "epsco-india", last_checked: 2026-09-07, status: stale}
contact_route: "info@epscoindia.com / +91 98204 20126 (Sunflower Building, Dindoshi Industrial Premises, GM Link Road, Goregaon East, Mumbai 400063). Founder & CEO Suhel Parker — US-certified/licensed asbestos inspector, planner and contractor; ~20–30 yrs of abatement work across the Indian subcontinent and Middle East."
followed: true
followed_date: 2026-09-08
last_checked: 2026-09-07
updated: 2026-09-08
---

# EPSCO India

**What they do.** Mumbai environmental-remediation contractor — asbestos abatement, inspection and testing, lead abatement, soil/chemical/oil-spill remediation, mercury cleanup, fire/flood restoration, HVAC duct cleaning. Founder & CEO **Suhel Parker** is a US-certified/licensed asbestos inspector, planner and contractor and runs NAETI abatement training in India (own record: `suhel-parker`). The remedy-side enterprise for asbestos *removal* — but its paying customers are hospitals, airports, pharma and food plants, data centres and power stations, i.e. corporate/institutional facilities, not the >100M mostly-Dalit people living under in-place AC roofs who carry the exposure mass.

## Scope
- `ambient-asbestos-demolition-dust` — a competent licensed abatement contractor *does* exist in India, which the stub otherwise implies is absent. It fills the enterprise leg for the **commercial-building segment only**; the residential / informal-demolition segment where the fibre-release mass sits has no actor and no payer, so EPSCO sharpens the gap rather than closing it (served population ≠ harmed population — the §4 over-served diagnostic).

## What they can offer
Removal, survey and disposal capacity that currently fires only on voluntary corporate procurement; an in-country certified trainer.

## What they need
A regulatory trigger — a pre-demolition ACM survey / licensed-abatement requirement in the C&D Waste Management Rules — is the missing demand signal.

## How to reach them
info@epscoindia.com; epscoindia.com. Suhel Parker via the company.

## Recent updates
- 2026-09-07 — Record created from a commercial-actor lead against the asbestos demolition-dust stub. — https://epscoindia.com/
