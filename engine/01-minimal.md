...
- **Two leaf ids are referenced by an actor and have no file.** `asbestos-import-legal` and `ambient-asbestos-demolition-dust`, both from `actor/gopal-krishna`. They migrated as stubs with no parent rather than being dropped, which is the stub design working as intended, and they are the store's only standing validation debt.
