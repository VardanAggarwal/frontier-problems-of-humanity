...
# `_NAME_TOKEN_STOPWORDS`, `_name_tokens`, `_text_mentions_actor` moved to
# `worker/identity.py` 2026-09-19 (reused unchanged by `passages.py`'s
# candidate-identity gate — see that module's `select()` docstring for the
# second bug this same check now catches). Imported at the top of this file;
# kept referenced here as a pointer for anyone grepping this module for them.
